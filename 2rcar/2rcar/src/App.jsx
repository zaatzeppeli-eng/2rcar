...
const EMPTY_FILTERS = { features:[], fuel:"", cambio:"", yearFrom:"", yearTo:"", kmMax:"", priceMax:"" };
const EMPTY_FORM = { brand:"", model:"", year:"", price:"", km:"", fuel:"Benzina", cambio:"manuale", type:"vendita", features:[], featured:false, description:"", status:"disponibile" };
const FALLBACK_CARS = [
...
// ─────────────────────────────────────────────────────────────
//  GLOBAL CSS — all animations GPU-accelerated (transform+opacity)
// ─────────────────────────────────────────────────────────────
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    --navy: #1a1a2e; --gold: #c9a84c; --gold-light: #e8c96a;
    --cream: #f7f5f0; --white: #ffffff; --text: #1a1a1a;
    --muted: #888888; --border: #e8e4dc;
  ...
  body { font-family:'DM Sans',sans-serif; background:var(--cream); color:var(--text); min-height:100vh; -webkit-font-smoothing:antialiased; overflow-x:hidden; }

  ::-webkit-scrollbar { width:6px; height:6px; }
  ::-webkit-scrollbar-track { background:transparent; }
  ::-webkit-scrollbar-thumb { background:linear-gradient(180deg,var(--gold) 0%,var(--navy) 100%); border-radius:99px; }
  ...
  * { scrollbar-width:thin; scrollbar-color:var(--gold) transparent; }
  ...
  @keyframes page-in {
    0%  { opacity:0; transform:translateY(18px) scale(0.994); filter:blur(2px); }
    100%{ opacity:1; transform:translateY(0) scale(1); filter:blur(0); }
  }
  @media (prefers-reduced-motion:reduce) {
    .page-transition { animation:page-in-reduced 0.2s ease both; }
    @keyframes page-in-reduced { from{opacity:0} to{opacity:1} }
  }
  ...
  .nav-bar {
    ...
    box-shadow:0 1px 0 rgba(201,168,76,0.08), 0 4px 32px rgba(0,0,0,0.22);
    height:var(--nav-h);
  ...
  .nav-bar.scrolled { background:rgba(20,20,36,0.99); box-shadow:0 1px 0 rgba(201,168,76,0.12),0 8px 40px rgba(0,0,0,0.3); }
  ...
  /* ── HERO ── */
  .hero {
    position:relative; height:clamp(380px,55vh,640px);
  ...
  .hero-eyebrow {
    font-family:'DM Sans',sans-serif; color:var(--gold); letter-spacing:0.28em;
    font-size:0.7rem; text-transform:uppercase; font-weight:500; margin-bottom:0.75rem;
    opacity:0; transform:translateY(12px);
    animation:hero-line 0.7s var(--ease-out-expo) 0.1s forwards;
  }
  .hero-title {
    font-family:'Playfair Display',serif; color:#fff;
    font-size:clamp(2rem,5vw,3.6rem); font-weight:900; line-height:1.15; margin-bottom:2rem;
    text-shadow:0 2px 24px rgba(0,0,0,0.4); letter-spacing:-0.01em;
    opacity:0; transform:translateY(20px);
    animation:hero-line 0.8s var(--ease-out-expo) 0.22s forwards;
  }
  .hero-btns {
    display:flex; gap:1rem; flex-wrap:wrap;
    opacity:0; transform:translateY(16px);
    animation:hero-line 0.7s var(--ease-out-expo) 0.38s forwards;
  }
  @keyframes hero-line { to { opacity:1; transform:translateY(0); } }

  .hero-btn { background:var(--gold); border:none; color:var(--navy); padding:0.85em 2.2em; font-family:'DM Sans',sans-serif; font-size:0.9rem; font-weight:700; cursor:pointer; border-radius:3px; letter-spacing:0.06em; text-transform:uppercase; transition:background 0.2s, transform 0.2s var(--ease-spring), box-shadow 0.2s; position:relative; overflow:hidden; }
  .hero-btn::after { content:''; position:absolute; inset:0; background:rgba(255,255,255,0); transition:background 0.15s; }
  .hero-btn:hover { background:var(--gold-light); transform:translateY(-2px) scale(1.02); box-shadow:0 8px 24px rgba(201,168,76,0.35); }
  ...
  .hero-btn-outline:hover { background:rgba(255,255,255,0.1); transform:translateY(-2px) scale(1.02); box-shadow:0 8px 24px rgba(255,255,255,0.1); }
  ...
  /* ── CAR CARD ── */
  .car-card {
    background:var(--white); border-radius:0.875rem; overflow:hidden; cursor:pointer;
  ...
  .car-card::before {
    ...
    background:linear-gradient(135deg,rgba(201,168,76,0) 0%,rgba(201,168,76,0.04) 100%);
    opacity:0; transition:opacity 0.3s ease; pointer-events:none; z-index:1;
  }
  .car-card:hover { transform:translateY(-6px) scale(1.008); box-shadow:0 20px 50px rgba(0,0,0,0.14), 0 0 0 1px rgba(201,168,76,0.12); }
  ...
  .card-img { width:100%; height:100%; object-fit:cover; transition:transform 0.55s var(--ease-out-expo), filter 0.3s ease; display:block; }
  .car-card:hover .card-img { transform:scale(1.06); }
  .card-badge { position:absolute; top:0.75rem; right:0.75rem; color:#fff; font-size:0.65rem; font-weight:700; padding:0.3em 0.75em; border-radius:999px; letter-spacing:0.08em; text-transform:uppercase; font-family:'DM Sans',sans-serif; backdrop-filter:blur(8px); }
  .photo-dots { position:absolute; bottom:0.6rem; left:50%; transform:translateX(-50%); display:flex; gap:0.3rem; }
  .photo-dot { width:5px; height:5px; border-radius:50%; transition:background 0.2s, transform 0.2s; }

  .card-body { padding:1.25rem; position:relative; z-index:2; }
  ...
  .card-price { font-family:'Playfair Display',serif; font-size:1.35rem; font-weight:700; color:var(--gold); transition:color 0.2s; }
  .car-card:hover .card-price { color:var(--gold-light); }
  ...
  .tag { background:#f0ede6; color:#666; font-size:0.7rem; padding:0.25em 0.65em; border-radius:999px; font-family:'DM Sans',sans-serif; font-weight:500; letter-spacing:0.03em; transition:background 0.15s, color 0.15s; }
  ...
  .filters-bar { display:flex; align-items:center; justify-content:space-between; background:var(--white); border:1px solid var(--border); border-radius:0.6rem; padding:0.7rem 1rem; cursor:pointer; user-select:none; transition:border-color 0.2s, box-shadow 0.2s; }
  .filters-bar:hover { border-color:var(--gold); box-shadow:0 0 0 3px rgba(201,168,76,0.08); }
  ...
  .filters-arrow { font-size:0.8rem; color:#aaa; display:inline-block; transition:transform 0.25s var(--ease-out-expo); }
  .filters-panel { background:var(--white); border:1px solid var(--border); border-top:none; border-radius:0 0 0.6rem 0.6rem; padding:1.1rem; display:grid; grid-template-columns:repeat(auto-fill,minmax(160px,1fr)); gap:0.85rem 1rem; animation:panel-open 0.3s var(--ease-out-expo); }
  ...
  .filter-tag-btn { background:var(--white); border:1px solid var(--border); color:#666; padding:0.3em 0.85em; border-radius:999px; cursor:pointer; font-size:0.78rem; font-family:'DM Sans',sans-serif; font-weight:500; transition:all 0.2s var(--ease-spring); }
  .filter-tag-btn:hover { border-color:var(--gold); color:var(--navy); transform:scale(1.03); }
  .filter-tag-btn.active { background:var(--navy); border-color:var(--navy); color:var(--gold); transform:scale(1.03); }
  .filters-summary { font-size:0.8rem; color:var(--muted); margin-top:0.6rem; min-height:1.2rem; }

  /* ── CTA ── */
  .cta-strip { background:var(--navy); padding:clamp(2rem,5vw,3.5rem) 1.5rem; }
  ...
  .cta-button { background:var(--gold); border:none; color:var(--navy); padding:0.85em 2em; font-family:'DM Sans',sans-serif; font-size:0.9rem; font-weight:700; cursor:pointer; border-radius:3px; letter-spacing:0.06em; text-transform:uppercase; white-space:nowrap; transition:background 0.2s,transform 0.2s var(--ease-spring),box-shadow 0.2s; flex-shrink:0; }
  .cta-button:hover { background:var(--gold-light); transform:translateY(-2px) scale(1.02); box-shadow:0 8px 24px rgba(201,168,76,0.3); }
  ...
  .contact-item { display:flex; align-items:center; gap:1rem; font-size:1rem; line-height:1.4; transition:transform 0.2s var(--ease-spring); cursor:default; }
  ...
  .overlay {
    ...
    animation:overlay-in 0.35s var(--ease-out-expo) forwards;
  }
  @keyframes overlay-in {
    to { background:rgba(0,0,0,0.65); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); }
  }
  .modal {
    background:var(--white); border-radius:1.125rem; width:100%; max-width:36rem;
  ...
  .modal.closing {
    animation:modal-out 0.28s var(--ease-in-out-quart) forwards;
  }
  @keyframes modal-out {
    to { opacity:0; transform:scale(0.94) translateY(16px); }
  }
  .modal-close { position:absolute; top:1rem; right:1rem; background:rgba(0,0,0,0.06); border:none; border-radius:50%; width:2.2rem; height:2.2rem; cursor:pointer; z-index:10; font-size:0.85rem; display:flex; align-items:center; justify-content:center; transition:background 0.15s, transform 0.2s var(--ease-spring); }
  .modal-close:hover { background:rgba(0,0,0,0.14); transform:scale(1.12) rotate(90deg); }

  .modal-gallery { position:relative; height:15rem; overflow:hidden; border-radius:1.125rem 1.125rem 0 0; background:#111; user-select:none; cursor:zoom-in; }
  .modal-gallery-img { width:100%; height:100%; object-fit:cover; display:block; transition:opacity 0.28s ease, transform 0.55s var(--ease-out-expo); }
  .modal-gallery-img.switching { opacity:0; transform:scale(1.03); }
  .modal-gallery-nav { position:absolute; top:50%; transform:translateY(-50%); background:rgba(0,0,0,0.38); border:none; color:#fff; border-radius:50%; width:2.4rem; height:2.4rem; font-size:1.5rem; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background 0.15s, transform 0.2s var(--ease-spring); z-index:5; backdrop-filter:blur(4px); }
  .modal-gallery-nav:hover { background:rgba(0,0,0,0.65); transform:translateY(-50%) scale(1.1); }
  ...
  .modal-gallery-dot { width:6px; height:6px; border-radius:50%; transition:background 0.2s, transform 0.2s var(--ease-spring), width 0.2s var(--ease-out-expo); cursor:pointer; }
  .modal-gallery-dot.active { width:18px; border-radius:3px; }
  .modal-gallery-zoom-hint { position:absolute; top:0.6rem; left:0.75rem; background:rgba(0,0,0,0.45); color:rgba(255,255,255,0.8); font-size:0.65rem; padding:0.2em 0.6em; border-radius:999px; pointer-events:none; backdrop-filter:blur(4px); }
  ...
  .contact-call-btn { display:block; background:var(--navy); color:var(--gold); text-align:center; padding:0.9em; border-radius:0.6rem; font-weight:700; text-decoration:none; font-size:1rem; letter-spacing:0.04em; transition:background 0.18s, transform 0.2s var(--ease-spring), box-shadow 0.2s; }
  ...
  /* ── GALLERY LIGHTBOX ── */
  .gallery-overlay { position:fixed; inset:0; background:rgba(0,0,0,0); z-index:2000; display:flex; align-items:center; justify-content:center; animation:gallery-bg-in 0.3s ease forwards; }
  @keyframes gallery-bg-in { to { background:rgba(0,0,0,0.96); } }
  .gallery-box { position:relative; max-width:56rem; width:100%; padding:3rem 1.5rem 1.5rem; animation:modal-in 0.35s var(--ease-out-expo); }
  .gallery-close { position:absolute; top:0.5rem; right:0.75rem; background:rgba(255,255,255,0.1); border:none; color:#fff; border-radius:50%; width:2.25rem; height:2.25rem; cursor:pointer; font-size:1.1rem; z-index:10; display:flex; align-items:center; justify-content:center; transition:background 0.15s, transform 0.2s var(--ease-spring); }
  .gallery-close:hover { background:rgba(255,255,255,0.2); transform:scale(1.1) rotate(90deg); }
  .gallery-main { width:100%; max-height:65vh; object-fit:contain; border-radius:0.5rem; display:block; margin:0 auto; }
  .gallery-nav { position:absolute; top:50%; transform:translateY(-50%); background:rgba(255,255,255,0.1); border:none; color:#fff; border-radius:50%; width:2.75rem; height:2.75rem; font-size:1.75rem; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background 0.15s, transform 0.2s var(--ease-spring); backdrop-filter:blur(4px); }
  ...
  .gallery-thumb { width:4rem; height:3rem; object-fit:cover; border-radius:0.25rem; cursor:pointer; opacity:0.4; transition:opacity 0.2s, transform 0.2s var(--ease-spring), outline 0.15s; }
  .gallery-thumb:hover { opacity:0.75; transform:scale(1.06); }
  .gallery-thumb.active { opacity:1; outline:2px solid var(--gold); transform:scale(1.06); }
  ...
  .auth-error { background:#fdecea; color:#e74c3c; border-radius:0.4rem; padding:0.5em 0.75em; font-size:0.82rem; text-align:center; width:100%; animation:shake 0.4s var(--ease-spring); }
  ...
  .logout-btn { background:#f8f8f8; border:1px solid var(--border); color:#666; padding:0.4em 0.9em; border-radius:0.4rem; cursor:pointer; font-size:0.82rem; font-family:'DM Sans',sans-serif; transition:background 0.15s, transform 0.15s; }
  .logout-btn:hover { background:#eee; transform:scale(1.02); }
  ...
  .feature-picker-btn { background:var(--white); border:1px solid var(--border); padding:0.3em 0.75em; border-radius:999px; cursor:pointer; font-size:0.75rem; font-family:'DM Sans',sans-serif; transition:all 0.2s var(--ease-spring); }
  .feature-picker-btn:hover { border-color:var(--gold); transform:scale(1.04); }
  .feature-picker-btn.active { background:var(--navy); border-color:var(--navy); color:var(--gold); transform:scale(1.04); }
  ...
  .dropzone.dragging { border-color:var(--gold); background:#fffbf0; transform:scale(1.01); }
  ...
  .preview-remove { position:absolute; top:-0.4rem; right:-0.4rem; background:#e74c3c; border:none; color:#fff; border-radius:50%; width:1.2rem; height:1.2rem; font-size:0.6rem; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:transform 0.15s var(--ease-spring); }
  .preview-remove:hover { transform:scale(1.2); }
  ...
  .submit-btn { background:var(--navy); color:var(--gold); border:none; padding:0.8em; border-radius:0.5rem; font-family:'DM Sans',sans-serif; font-size:0.95rem; font-weight:700; cursor:pointer; letter-spacing:0.04em; transition:background 0.18s, transform 0.2s var(--ease-spring), box-shadow 0.2s, opacity 0.18s; }
  .submit-btn:hover:not(:disabled) { background:#2a2a4e; transform:translateY(-1px); box-shadow:0 6px 20px rgba(26,26,46,0.2); }
  ...
  .admin-list-item { display:flex; align-items:center; justify-content:space-between; padding:0.7rem 0.9rem; background:#f8f8f8; border-radius:0.5rem; border:1px solid var(--border); transition:background 0.15s, transform 0.15s; }
  .admin-list-item:hover { background:#f0ede6; transform:translateX(2px); }
  ...
  .admin-action-btn { border:none; border-radius:0.4rem; width:2rem; height:2rem; cursor:pointer; font-size:0.75rem; transition:opacity 0.15s, transform 0.2s var(--ease-spring); display:flex; align-items:center; justify-content:center; font-weight:700; }
  .admin-action-btn:hover { opacity:0.8; transform:scale(1.1); }
  .edit-modal { background:var(--white); border-radius:1rem; width:100%; max-width:40rem; max-height:92vh; overflow-y:auto; position:relative; box-shadow:0 24px 80px rgba(0,0,0,0.3); animation:modal-in 0.4s var(--ease-out-expo); }
  ...
  .stat-block { padding:1.75rem 1rem; text-align:center; border-right:1px solid rgba(26,26,46,0.15); transition:background 0.2s; cursor:default; }
  .stat-block:hover { background:rgba(255,255,255,0.12); }
  ...
  .value-card { background:var(--white); border:1px solid var(--border); border-radius:0.75rem; padding:1.5rem 1.25rem; box-shadow:var(--card-shadow); transition:transform 0.3s var(--ease-spring), box-shadow 0.3s; }
  .value-card:hover { transform:translateY(-4px); box-shadow:0 12px 32px rgba(0,0,0,0.1); }
  ...
  .trust-badge { display:flex; align-items:center; gap:0.6rem; background:var(--white); border:1px solid var(--border); border-radius:999px; padding:0.5em 1.2em; font-size:0.82rem; font-weight:500; color:var(--navy); box-shadow:0 2px 8px rgba(0,0,0,0.05); transition:transform 0.2s var(--ease-spring), box-shadow 0.2s, border-color 0.2s; }
  .trust-badge:hover { transform:translateY(-2px) scale(1.03); box-shadow:0 6px 20px rgba(0,0,0,0.08); border-color:rgba(201,168,76,0.4); }
  ...
  .mobile-menu {
    ...
    background:rgba(18,18,32,0.98); backdrop-filter:blur(16px);
    z-index:199; padding:0.75rem 1rem 1.25rem;
    display:flex; flex-direction:column; gap:0.25rem;
    box-shadow:0 8px 32px rgba(0,0,0,0.4);
    animation:menu-slide 0.3s var(--ease-out-expo);
  ...
  .mobile-menu-item {
    ...
    padding:0.85em 1em; border-radius:0 0.4rem 0.4rem 0;
    text-align:left; font-family:'DM Sans',sans-serif; font-weight:400;
    letter-spacing:0.03em; transition:all 0.18s var(--ease-out-expo);
  }
  .mobile-menu-item:hover { color:#fff; background:rgba(201,168,76,0.06); transform:translateX(3px); }
  .mobile-menu-item.active { background:rgba(201,168,76,0.12); border-left-color:#c9a84c; color:#c9a84c; font-weight:600; }
  ...
  @media (max-width: 768px) { .nav-items-desktop { display:none !important; } }
  ...
  .intro-bg-core { background:radial-gradient(ellipse 72% 55% at 50% 50%,rgba(10,28,68,0.72) 0%,transparent 72%); animation:intro-breathe 9s ease-in-out infinite; }
  .intro-bg-tl { background:radial-gradient(ellipse 38% 28% at 14% 17%,rgba(16,42,95,0.22) 0%,transparent 70%); animation:intro-drift 13s ease-in-out infinite alternate; }
  .intro-bg-br { background:radial-gradient(ellipse 32% 22% at 85% 81%,rgba(22,48,108,0.16) 0%,transparent 70%); animation:intro-drift 16s ease-in-out infinite alternate-reverse; }
  .intro-bg-vig { background:radial-gradient(ellipse 100% 100% at 50% 50%,transparent 36%,rgba(2,6,18,0.82) 100%); }
  .intro-halo { position:absolute; width:clamp(220px,60vw,340px); height:clamp(220px,60vw,340px); border-radius:50%; background:radial-gradient(circle,rgba(201,168,76,0.1) 0%,rgba(201,168,76,0.035) 48%,transparent 68%); top:50%; left:50%; transform:translate(-50%,-50%); pointer-events:none; animation:intro-halo 4.5s ease-in-out 2.8s infinite; }
  .intro-logo-wrap { position:relative; z-index:10; display:flex; flex-direction:column; align-items:center; gap:20px; opacity:0; transform:scale(0.90) translateY(12px); filter:blur(12px); will-change:opacity,transform,filter; animation:intro-emerge 2s cubic-bezier(0.22,1,0.36,1) 0.7s forwards; }
  ...
  .intro-wm-brand { font-family:'Playfair Display',serif; font-size:clamp(30px,9vw,44px); font-weight:900; letter-spacing:0.02em; color:#c9a84c; line-height:1; }
  .intro-wm-car { font-family:'DM Sans',sans-serif; font-size:clamp(13px,3.5vw,20px); font-weight:300; letter-spacing:0.35em; color:rgba(201,168,76,0.62); line-height:1; padding-left:clamp(7px,2vw,12px); text-transform:uppercase; align-self:flex-end; margin-bottom:2px; }
  .intro-wm-sub { font-family:'DM Sans',sans-serif; font-size:clamp(7.5px,2vw,9.5px); font-weight:400; letter-spacing:0.45em; color:rgba(201,168,76,0.25); text-transform:uppercase; line-height:1; }
  .intro-sep { width:0; height:1px; background:linear-gradient(90deg,transparent,rgba(201,168,76,0.35),transparent); animation:intro-expand 1.1s cubic-bezier(0.22,1,0.36,1) 2.2s forwards; }
  .intro-eyebrow { font-family:'DM Sans',sans-serif; font-size:clamp(7px,1.8vw,9px); letter-spacing:0.5em; font-weight:400; color:rgba(201,168,76,0.22); text-transform:uppercase; opacity:0; animation:intro-subfade 1.2s ease 2.6s forwards; }
  ...
  @keyframes intro-emerge { 0%{opacity:0;transform:scale(0.90) translateY(12px);filter:blur(12px)} 58%{opacity:1;filter:blur(0.6px)} 100%{opacity:1;transform:scale(1) translateY(0);filter:blur(0)} }
  @keyframes intro-halo { 0%,100%{opacity:0.5;transform:translate(-50%,-50%) scale(1)} 50%{opacity:1;transform:translate(-50%,-50%) scale(1.14)} }
  @keyframes intro-expand { 0%{width:0} 100%{width:min(160px,42vw)} }
  @keyframes intro-subfade { 0%{opacity:0} 100%{opacity:1} }

  @media (max-width:480px) { .intro-logo-wrap{gap:14px} .intro-bg-core{background:radial-gradient(ellipse 95% 70% at 50% 50%,rgba(10,28,68,0.68) 0%,transparent 72%)} }
  @media (max-width:360px) { .intro-wm-sub{letter-spacing:0.3em} .intro-eyebrow{letter-spacing:0.32em} }
  @media (max-height:500px) and (orientation:landscape) { .intro-logo-wrap{gap:10px;flex-direction:row;flex-wrap:wrap;justify-content:center} .intro-mark{width:48px;height:48px} .intro-wordmark{gap:3px} .intro-wm-brand{font-size:26px} .intro-wm-car{font-size:13px} .intro-sep,.intro-eyebrow{display:none} .intro-halo{width:160px;height:160px} }
  @media (prefers-reduced-motion:reduce) {
...
function AdvancedFilters({ filters, onChange, type, resultCount }) {
  ...
  const activeCount =
    (filters.fuel?1:0)+(filters.cambio?1:0)+(filters.yearFrom?1:0)+
    (filters.yearTo?1:0)+(filters.kmMax?1:0)+(filters.priceMax?1:0)+
    (filters.features?.length||0);
  function set(key, val) { onChange({ ...filters, [key]: val }); }
  function toggleFeature(f) {
    const cur = filters.features||[];
    onChange({ ...filters, features: cur.includes(f)?cur.filter(x=>x!==f):[...cur,f] });
  }
  function resetAll(e) { e.stopPropagation(); onChange({ ...EMPTY_FILTERS }); }
  const PRICE_V=[["3000","€3k"],["5000","€5k"],["8000","€8k"],["10000","€10k"],["15000","€15k"],["20000","€20k"],["30000","€30k"]];
  const PRICE_N=[["30","€30/g"],["50","€50/g"],["80","€80/g"],["120","€120/g"],["200","€200/g"]];
  const priceOpts = type==="noleggio"?PRICE_N:PRICE_V;
  const summaryParts=[];
  ...
  return (
    <div className="filters-wrapper">
      <div className="filters-bar" onClick={()=>setOpen(o=>!o)} style={{borderRadius:open?"0.6rem 0.6rem 0 0":"0.6rem"}}>
      ...
      {open&&(
        <div className="filters-panel">
          <div className="filter-group"><label>Carburante</label><select className="filter-select" value={filters.fuel} onChange={e=>set("fuel",e.target.value)}><option value="">Tutti</option>{["Benzina","Diesel","Ibrida","Elettrica","GPL"].map(f=><option key={f}>{f}</option>)}</select></div>
          <div className="filter-group"><label>Cambio</label><select className="filter-select" value={filters.cambio} onChange={e=>set("cambio",e.target.value)}><option value="">Tutti</option><option value="manuale">Manuale</option><option value="automatico">Automatico</option></select></div>
          <div className="filter-group"><label>Anno da</label><select className="filter-select" value={filters.yearFrom} onChange={e=>set("yearFrom",e.target.value)}><option value="">–</option>{Array.from({length:16},(_,i)=>2025-i).map(y=><option key={y}>{y}</option>)}</select></div>
          <div className="filter-group"><label>Anno a</label><select className="filter-select" value={filters.yearTo} onChange={e=>set("yearTo",e.target.value)}><option value="">–</option>{Array.from({length:16},(_,i)=>2025-i).map(y=><option key={y}>{y}</option>)}</select></div>
          <div className="filter-group"><label>Km max</label><select className="filter-select" value={filters.kmMax} onChange={e=>set("kmMax",e.target.value)}><option value="">Tutti</option>{[["20000","20k"],["50000","50k"],["80000","80k"],["120000","120k"],["200000","200k"]].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div>
          <div className="filter-group"><label>Prezzo max</label><select className="filter-select" value={filters.priceMax} onChange={e=>set("priceMax",e.target.value)}><option value="">Tutti</option>{priceOpts.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div>
          <hr className="filter-divider"/>
          <div className="filter-features-area"><span className="filter-features-label">Caratteristiche</span><div className="filter-features-row">{FEATURES_AVAILABLE.map(f=><button key={f} className={`filter-tag-btn${filters.features?.includes(f)?" active":""}`} onClick={()=>toggleFeature(f)}>{f}</button>)}</div></div>
        </div>
      )}
      {summaryParts.length>0&&<p className="filters-summary">Filtri: {summaryParts.join(" · ")} — {resultCount} risultati</p>}
    </div>
  );
}
...
function ImageUploader({ previews, setPreviews, setUploadedFiles, uploading }) {
  ...
  function handleFiles(files) {
    ...
    setPreviews(prev=>[...prev,...imgs.map(f=>({url:URL.createObjectURL(f),file:f}))]);
    setUploadedFiles(prev=>[...prev,...imgs]);
  }
  function removePreview(i) { setPreviews(p=>p.filter((_,j)=>j!==i)); setUploadedFiles(p=>p.filter((_,j)=>j!==i)); }
  return (
    <div>
      <div className={`dropzone${dragging?" dragging":""}`}
        ...
        <p className="dropzone-text">{uploading?"⏳ Upload in corso...":(<>📷 Trascina le foto qui oppure clicca<br/><span style={{fontSize:"0.75rem",color:"#bbb"}}>Puoi caricare più foto alla volta</span></>)}</p>
      </div>
      {previews.length>0&&<div className="preview-grid">{previews.map((item,i)=><div key={i} className="preview-item"><img src={item.url} alt="" className="preview-img"/><button type="button" className="preview-remove" onClick={()=>removePreview(i)}>✕</button>{i===0&&<span className="preview-main">Copertina</span>}</div>)}</div>}
    </div>
  );
}
...
function AdminCarForm({ initial, onSubmit, saving, uploading, title, submitLabel }) {
  ...
  const [previews, setPreviews] = useState((initial?.images||[]).map(url=>({url,file:null})));
  const [newFiles, setNewFiles] = useState([]);
  useEffect(()=>{ if(initial){ setForm(initial); setPreviews((initial.images||[]).map(url=>({url,file:null}))); setNewFiles([]); } },[initial?.id]);
  function toggleFeature(f) { setForm(prev=>({...prev,features:prev.features.includes(f)?prev.features.filter(x=>x!==f):[...prev.features,f]})); }
  async function handleSubmit(e) { e.preventDefault(); const existing=previews.filter(p=>!p.file).map(p=>p.url); await onSubmit(form,newFiles,existing); }
  return (
    <div style={{padding:"1.75rem"}}>
      <p className="admin-section-title">{title}</p>
      <form onSubmit={handleSubmit} className="admin-form">
        <div className="form-row"><input required placeholder="Marca" value={form.brand} onChange={e=>setForm({...form,brand:e.target.value})} className="input"/><input required placeholder="Modello" value={form.model} onChange={e=>setForm({...form,model:e.target.value})} className="input"/></div>
        <div className="form-row"><input required type="number" placeholder="Anno" value={form.year} onChange={e=>setForm({...form,year:e.target.value})} className="input"/><input required type="number" placeholder={form.type==="noleggio"?"€/giorno":"Prezzo €"} value={form.price} onChange={e=>setForm({...form,price:e.target.value})} className="input"/></div>
        <div className="form-row"><input required type="number" placeholder="Km" value={form.km} onChange={e=>setForm({...form,km:e.target.value})} className="input"/><select value={form.fuel} onChange={e=>setForm({...form,fuel:e.target.value})} className="select-field">{["Benzina","Diesel","Ibrida","Elettrica","GPL"].map(f=><option key={f}>{f}</option>)}</select></div>
        <div className="form-row">
          <select required value={form.cambio} onChange={e=>setForm({...form,cambio:e.target.value})} className="select-field"><option value="">-- Cambio (obbligatorio) --</option><option value="manuale">Manuale</option><option value="automatico">Automatico</option></select>
          <select value={form.type} onChange={e=>{ const t=e.target.value; setForm(prev=>({...prev,type:t,status:(t!=="noleggio"&&prev.status==="noleggiata")?"disponibile":prev.status})); }} className="select-field"><option value="vendita">Vendita</option><option value="noleggio">Noleggio</option></select>
        </div>
        <select value={form.status} onChange={e=>setForm({...form,status:e.target.value})} className="select-field"><option value="disponibile">Disponibile</option><option value="venduta">Venduta</option>{form.type==="noleggio"&&<option value="noleggiata">Noleggiata</option>}</select>
        ...
        <div className="feature-picker"><span className="feature-picker-label">Caratteristiche</span><div className="feature-picker-grid">{FEATURES_AVAILABLE.map(f=><button type="button" key={f} onClick={()=>toggleFeature(f)} className={`feature-picker-btn${form.features.includes(f)?" active":""}`}>{f}</button>)}</div></div>
        <span className="feature-picker-label" style={{marginBottom:"0.25rem"}}>Foto</span>
        <ImageUploader previews={previews} setPreviews={setPreviews} setUploadedFiles={setNewFiles} uploading={uploading}/>
        <label className="check-label"><input type="checkbox" checked={form.featured} onChange={e=>setForm({...form,featured:e.target.checked})}/> Metti in vetrina</label>
        <button type="submit" className="submit-btn" style={{opacity:(saving||uploading)?0.6:1}} disabled={saving||uploading}>{saving?"Salvataggio...":submitLabel||"Salva"}</button>
      </form>
    </div>
  );
}
...
// ─────────────────────────────────────────────────────────────
//  MODAL GALLERY — with smooth image transitions
// ─────────────────────────────────────────────────────────────
function ModalGallery({ images, onZoom }) {
  ...
  function go(nextIdx) {
    if (switching || nextIdx === idx) return;
    setSwitching(true);
    setTimeout(() => { setIdx(nextIdx); setSwitching(false); }, 220);
  }
  function prev(e) { e.stopPropagation(); go((idx-1+imgs.length)%imgs.length); }
  function next(e) { e.stopPropagation(); go((idx+1)%imgs.length); }
  return (
    <div className="modal-gallery" onClick={()=>onZoom(idx)}>
      <img src={imgs[idx]} alt="" className={`modal-gallery-img${switching?" switching":""}`}/>
      {imgs.length>1&&(
        <>
          ...
          <div className="modal-gallery-dots">
            {imgs.map((_,i)=>(
              <div key={i} className={`modal-gallery-dot${i===idx?" active":""}`}
                style={{background:i===idx?"#c9a84c":"rgba(255,255,255,0.55)"}}
...
// ─────────────────────────────────────────────────────────────
//  CAR CARD — with reveal animation
// ─────────────────────────────────────────────────────────────
function CarCard({ car, onClick, revealDelay = 0 }) {
  ...
  function handleMouseEnter() {
    ...
    let i=0; intervalRef.current=setInterval(()=>{i=(i+1)%images.length;setImgIdx(i);},700);
  }
  ...
  return (
    ...
    >
      <div className="card-img-wrap">
        ...
        {isNoleggiata&&<div className="noleggiata-overlay"><div className="noleggiata-stamp">NOLEGGIATA</div></div>}
        {images.length>1&&isAvailable&&(
          <div className="photo-dots">{images.map((_,i)=><div key={i} className="photo-dot" style={{background:i===imgIdx?"#c9a84c":"rgba(255,255,255,0.6)"}}/>)}</div>
        )}
      </div>
      <div className="card-body">
        <div className="card-tags">
          <span className="card-cambio-badge">{car.cambio==="automatico"?"⚙️ Automatico":"🔧 Manuale"}</span>
        ...
        {!isSold&&<p className="card-price">{car.type==="noleggio"?<>€ {car.price}<span className="card-price-sub">/giorno</span></>:`€ ${car.price?.toLocaleString()}`}</p>}
      ...
      <div className="gallery-box" onClick={e=>e.stopPropagation()}>
        ...
        {imgs.length>1&&(
          <>
            ...
            <div className="gallery-thumbs">{imgs.map((url,i)=><img key={i} src={url} alt="" onClick={()=>setIdx(i)} className={`gallery-thumb${i===idx?" active":""}`}/>)}</div>
  ...
  return (
    <>
      <div className="chisiamo-hero">
        ...
        <p className="chisiamo-hero-sub">Siamo un autosalone indipendente a Roma, nato dalla passione per le auto e dal rispetto per i clienti. Niente fronzoli, niente sorprese: solo auto controllate e prezzi chiari.</p>
      </div>
      <div className="stats-row">
        {[{num:"15+",label:"Anni di esperienza"},{num:"800+",label:"Auto vendute"},{num:"0%",label:"Stress"},{num:"100%",label:"Trasparenza"}].map(s=>(
          <div key={s.label} className="stat-block"><span className="stat-num">{s.num}</span><span className="stat-label">{s.label}</span></div>
        ))}
      </div>
      <div className="section">
        <div style={{textAlign:"center",marginBottom:"2rem"}}>
          <h2 className="section-title reveal">I nostri valori</h2>
      ...
      <div className="section" style={{paddingBottom:"1rem"}}>
        <div style={{textAlign:"center",marginBottom:"1.5rem"}}>
          <h2 className="section-title reveal">Perché sceglierci</h2>
          <div className="ornament"><div className="ornament-line"/><div className="ornament-dot"/><div className="ornament-line"/></div>
        </div>
        <div className="trust-badges">
          {[["✅","Attività regolare con P.IVA"],["📍","Sede fisica verificabile a Roma"],["📞","Risposta telefonica garantita"],["⭐","Oltre 15 anni di esperienza"],["🗓️","Aperti dal lunedì al sabato"]].map(([icon,label],i)=>(
            <div key={label} className={`trust-badge reveal reveal-delay-${(i%3)+1}`}>
              <span className="trust-badge-icon">{icon}</span><span>{label}</span>
            </div>
          ))}
        </div>
      </div>
      <div className="cta-strip">
        <div className="cta-inner">
          <div><h3 className="cta-title">Vieni a trovarci senza impegno</h3><p className="cta-sub">Via Collatina 381, Roma — aperto lun–sab 9:00–19:00</p></div>
  ...
  return (
    <>
      <div className="nav-mobile-actions" style={{gap:"0.4rem",alignItems:"center"}}>
        <button style={{background:"rgba(201,168,76,0.15)",border:"1px solid rgba(201,168,76,0.5)",color:"#c9a84c",cursor:"pointer",fontSize:"0.78rem",padding:"0.4em 0.9em",borderRadius:"0.4rem",fontFamily:"'DM Sans',sans-serif",fontWeight:600,letterSpacing:"0.04em",lineHeight:1,transition:"all 0.2s"}} onClick={()=>setAdminOpen(true)}>
          {adminUser?"Admin ✓":"Login"}
        </button>
        <a href="tel:+393000008654" style={{background:"rgba(201,168,76,0.15)",border:"1px solid rgba(201,168,76,0.4)",color:"#c9a84c",borderRadius:"0.4rem",padding:"0.45em 0.7em",fontSize:"1.1rem",lineHeight:1,textDecoration:"none",display:"flex",alignItems:"center",transition:"all 0.2s"}} aria-label="Chiama">📞</a>
        <button style={{background:menuOpen?"rgba(201,168,76,0.15)":"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.18)",color:menuOpen?"#c9a84c":"#fff",borderRadius:"0.4rem",padding:"0.45em 0.65em",fontSize:"1.1rem",cursor:"pointer",lineHeight:1,display:"flex",alignItems:"center",transition:"all 0.2s"}} onClick={()=>setMenuOpen(o=>!o)} aria-label="Menu">
          {menuOpen?"✕":"☰"}
        </button>
      </div>
      {menuOpen&&(
        <div className="mobile-menu">
          {NAV_ITEMS.map(item=>(
            <button key={item} className={`mobile-menu-item${page===item?" active":""}`} onClick={()=>{navigateTo(item);setMenuOpen(false);}}>
...
export default function App() {
  ...
  useReveal(); // runs after every render to catch new .reveal elements
  ...
  async function handleAddCar(form,newFiles,existingUrls) {
    ...
    if(newFiles.length>0) { const up=await uploadImages(newFiles); imageUrls=[...imageUrls,...up]; }
    setUploading(false);
  ...
  async function toggleNoleggiata(id) {
    ...
    if(car.type!=="noleggio"){alert("Solo le auto di tipo NOLEGGIO possono essere marcate come noleggiata");return;}
    const ns=car.status==="noleggiata"?"disponibile":"noleggiata";
    setCars(prev=>prev.map(c=>c.id===id?{...c,status:ns}:c));
    if(dbConnected) await supabase.from("cars").update({status:ns,sold_at:null}).eq("id",id);
  }
  ...
  return (
    <div className="page-wrapper">
      ...
      {/* ── NAVBAR ── */}
      <nav className={`nav-bar${navHidden?" hidden":""}${navScrolled?" scrolled":""}`}>
        <div className="nav-inner">
          ...
          <div className="nav-items-desktop" style={{display:"flex",gap:"0.25rem",alignItems:"center"}}>
            {NAV_ITEMS.map(item=>(
              <div key={item} className={`nav-link-wrap${page===item?" active":""}`}>
                <button style={{background:page===item?"rgba(201,168,76,0.1)":"none",border:"none",color:page===item?"#c9a84c":"#bbb",cursor:"pointer",fontSize:"0.82rem",padding:"0.5em 0.9em",borderRadius:"0.4rem",letterSpacing:"0.04em",fontFamily:"'DM Sans',sans-serif",fontWeight:page===item?600:400,transition:"color 0.2s,background 0.2s"}} onClick={()=>navigateTo(item)}>{item}</button>
              </div>
            ))}
            <button style={{background:"rgba(201,168,76,0.15)",border:"1px solid rgba(201,168,76,0.5)",color:"#c9a84c",cursor:"pointer",fontSize:"0.78rem",padding:"0.4em 0.9em",borderRadius:"0.4rem",fontFamily:"'DM Sans',sans-serif",fontWeight:600,letterSpacing:"0.04em",marginLeft:"0.5rem",transition:"all 0.2s"}} onClick={()=>setAdminOpen(true)}>
              {adminUser?"Admin ✓":"Login"}
            </button>
          </div>
        ...
        {!loading&&(
          <PageTransition pageKey={page}>
              ...
              <>
                ...
                <section className="section">
                  <div className="section-header">
                    <h2 className="section-title reveal">In Vetrina</h2>
                    <div className="ornament"><div className="ornament-line"/><div className="ornament-dot"/><div className="ornament-line"/></div>
                    <p className="section-sub reveal">Le auto del momento selezionate per te</p>
                  </div>
                  {featuredCars.length===0
                    ?<p style={{textAlign:"center",color:"#aaa",padding:"2.5rem 0"}}>Nessuna auto in vetrina</p>
                    :<div className="grid-3">{featuredCars.map((car,i)=><CarCard key={car.id} car={car} onClick={()=>openCar(car)} revealDelay={i+1}/>)}</div>
                  }
                </section>
                <div className="cta-strip">
                  <div className="cta-inner">
                    <div className="reveal"><h3 className="cta-title">Hai bisogno di un'auto per qualche giorno?</h3><p className="cta-sub">Noleggio flessibile, disponibile subito.</p></div>
                    <button className="cta-button reveal" onClick={()=>navigateTo("Noleggio")}>Vedi il noleggio →</button>
                  </div>
                </div>
              </>
            )}
            ...
            {page==="Vendita"&&(
              <section className="section">
                <div className="section-header">
                  <h2 className="section-title reveal">Auto in Vendita</h2>
                  <div className="ornament"><div className="ornament-line"/><div className="ornament-dot"/><div className="ornament-line"/></div>
                </div>
                <AdvancedFilters filters={filters} onChange={setFilters} type="vendita" resultCount={filteredSale.length}/>
                {filteredSale.length===0?<EmptyState text="Nessuna auto corrisponde ai filtri selezionati."/>
                  :<div className="grid-3">{filteredSale.map((car,i)=><CarCard key={car.id} car={car} onClick={()=>openCar(car)} revealDelay={(i%3)+1}/>)}</div>}
              </section>
            )}
            ...
            {page==="Noleggio"&&(
              <section className="section">
                <div className="section-header">
                  <h2 className="section-title reveal">Auto a Noleggio</h2>
                  <div className="ornament"><div className="ornament-line"/><div className="ornament-dot"/><div className="ornament-line"/></div>
                  <p className="section-sub reveal">Prezzi al giorno — disponibilità immediata</p>
                </div>
                <AdvancedFilters filters={filters} onChange={setFilters} type="noleggio" resultCount={filteredRental.length}/>
                {filteredRental.length===0?<EmptyState text="Nessuna auto corrisponde ai filtri selezionati."/>
                  :<div className="grid-3">{filteredRental.map((car,i)=><CarCard key={car.id} car={car} onClick={()=>openCar(car)} revealDelay={(i%3)+1}/>)}</div>}
              </section>
            )}
            ...
            {page==="Contatti"&&(
              <section className="section">
                <div className="section-header">
                  <h2 className="section-title reveal">Contattaci</h2>
                  <div className="ornament"><div className="ornament-line"/><div className="ornament-dot"/><div className="ornament-line"/></div>
                  <p className="section-sub reveal">Siamo a tua disposizione</p>
                </div>
                <div className="contact-card reveal">
                  {[["📍","Via Collatina 381, Roma (RM)"],["📞","393 000 8654 — 06 88922000"],["✉️","2erreprofessionalcar@libero.it"],["🕐","Lun–Sab: 9:00–19:00"]].map(([icon,text])=>(
                    <div key={text} className="contact-item"><span className="contact-icon">{icon}</span><span>{text}</span></div>
                  ))}
                </div>
              </section>
            )}
        ...
        <Modal onClose={()=>{setSelectedCar(null);setGalleryOpen(false);}}>
          ...
          <div className="modal-body">
            <div className="modal-tags">
              <span className="tag" style={{background:selectedCar.status==="venduta"?"#e74c3c":selectedCar.status==="noleggiata"?"#7f8c8d":selectedCar.type==="noleggio"?"#e67e22":"#1a1a2e",color:"#fff"}}>
            ...
            <p className="modal-cambio">{selectedCar.cambio==="automatico"?"⚙️ Cambio Automatico":"🔧 Cambio Manuale"}</p>
            {(selectedCar.features||[]).length>0&&(
              <div style={{marginBottom:"1rem"}}>
                <p className="modal-features-title">Caratteristiche</p>
                <div className="modal-features-grid">{(selectedCar.features||[]).map(f=><Tag key={f} label={f}/>)}</div>
              </div>
            )}
      ...
      {editOpen&&editingCar&&(
        <Modal onClose={()=>{setEditOpen(false);setEditingCar(null);}} wide>
          <AdminCarForm initial={{...editingCar,features:editingCar.features||[],year:String(editingCar.year),price:String(editingCar.price),km:String(editingCar.km)}} onSubmit={handleEditCar} saving={saving} uploading={uploading} title={`✏️ Modifica: ${editingCar.brand} ${editingCar.model}`} submitLabel="Aggiorna Annuncio"/>
        </Modal>
      )}
          ...
          {!adminUser?(
            <div className="admin-login">
              ...
              <form onSubmit={handleAdminLogin} style={{width:"100%",display:"flex",flexDirection:"column",gap:"0.75rem"}}>
                ...
                <button type="submit" className="submit-btn" style={{opacity:authLoading?0.6:1}} disabled={authLoading}>{authLoading?"Accesso in corso...":"Entra"}</button>
              </form>
            </div>
          ):(
            <div className="admin-panel">
              <div className="admin-header">
                <div><h2 className="admin-title">Pannello Admin</h2><p className="admin-user-badge">✓ {adminUser.email}</p></div>
                <button className="logout-btn" onClick={handleAdminLogout}>Esci</button>
              </div>
              <AdminCarForm initial={EMPTY_FORM} onSubmit={handleAddCar} saving={saving} uploading={uploading} title="➕ Nuovo Annuncio" submitLabel="Aggiungi Annuncio"/>
                    ...
                    <div key={car.id} className="admin-list-item">
                      <div style={{display:"flex",alignItems:"center",gap:"0.75rem",minWidth:0}}>
                        ...
                        <div className="admin-list-info" style={{minWidth:0}}>
                          ...
                          <span className="admin-list-sub">{car.year} · {statusLabel(car)} · {car.cambio} · {car.images?.length||1} foto</span>
                        </div>
                      </div>
                      <div className="admin-list-actions">
                        <button className="admin-action-btn" title="Vetrina" style={{background:car.featured?"#f39c12":"#ecf0f1",color:car.featured?"#fff":"#333"}} onClick={()=>toggleFeatured(car.id)}>⭐</button>
                        <button className="admin-action-btn" style={{background:car.status==="venduta"?"#e74c3c":"#ecf0f1",color:car.status==="venduta"?"#fff":"#333",fontSize:"0.65rem",fontWeight:700}} onClick={()=>toggleSold(car.id)} title={car.status==="venduta"?"Segna come disponibile":"Segna come venduta"}>{car.status==="venduta"?"✓V":"VND"}</button>
                        <button className="admin-action-btn" style={{background:car.status==="noleggiata"?"#7f8c8d":"#ecf0f1",color:car.status==="noleggiata"?"#fff":"#333",fontSize:"0.65rem",fontWeight:700,opacity:car.type==="noleggio"?1:0.45,cursor:car.type==="noleggio"?"pointer":"not-allowed"}} onClick={()=>{if(car.type==="noleggio")toggleNoleggiata(car.id);}} title={car.type==="noleggio"?(car.status==="noleggiata"?"Segna come disponibile":"Segna come noleggiata"):"Solo per auto a noleggio"}>{car.status==="noleggiata"?"✓N":"NLG"}</button>
                        <button className="admin-action-btn" style={{background:"#3498db",color:"#fff"}} onClick={()=>{setEditingCar(car);setEditOpen(true);setAdminOpen(false);}} title="Modifica">✏️</button>
                        <button className="admin-action-btn" style={{background:car.status==="venduta"?"#e74c3c":"#ddd",color:car.status==="venduta"?"#fff":"#aaa",cursor:car.status==="venduta"?"pointer":"not-allowed"}} onClick={()=>deleteCar(car.id)} title={car.status==="venduta"?"Elimina":"Solo le auto vendute possono essere eliminate"}>✕</button>
                      </div>
                    </div>
                  ))}
                </div>
