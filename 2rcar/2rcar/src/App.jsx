...
// ─────────────────────────────────────────────────────────────
//  GLOBAL CSS — desktop invariato + blocco mobile aggiunto
// ─────────────────────────────────────────────────────────────
const GLOBAL_CSS = `
  ...
  * { scrollbar-width: thin; scrollbar-color: var(--gold) transparent; }
  ...
  /* ── WhatsApp FAB (nascosto su desktop, visibile su mobile) ── */
  .whatsapp-fab {
    display: none;
  }
  ...
  /* ══════════════════════════════════════════════════════════════
     MOBILE — ottimizzazione UX (≤ 768px)
     Regola: il desktop NON viene toccato da queste regole.
  ══════════════════════════════════════════════════════════════ */
  @media (max-width: 768px) {
    ...
    /* ── Navbar: più compatta, logo + icone essenziali ── */
    .nav-inner {
      padding: 0 1rem;
    }

    /* Nascondi le voci di navigazione testuali */
    .nav-items-desktop {
      display: none !important;
    }

    /* ── Hero: altezza ridotta, testo più piccolo ── */
    .hero {
    ...
    /* ── Grid auto → singola colonna ── */
    .grid-3 {
      grid-template-columns: 1fr;
      gap: 1rem;
    }

    /* ── Card: layout orizzontale su mobile (immagine a sinistra) ── */
    .car-card {
      display: flex;
      flex-direction: row;
      align-items: stretch;
    ...
    /* Foto-dots nascosti in layout orizzontale */
    .photo-dots {
      display: none;
    }
    .sold-overlay .sold-stamp {
      font-size: 1rem;
      border-width: 2px;
    }
    .card-body {
    ...
    /* ── CTA strip: stack verticale, testo più piccolo ── */
    .cta-strip {
      padding: 1.75rem 1rem;
    }
    .cta-inner {
    ...
    /* ── Filters: panel a colonna singola ── */
    .filters-panel {
      grid-template-columns: 1fr;
    }
    .filter-select {
    ...
    /* ── Modale: quasi full screen su mobile ── */
    .overlay {
      padding: 0;
      align-items: flex-end;
    }
    ...
    /* ── Gallery lightbox: nav più grandi ── */
    .gallery-nav {
      width: 3rem;
      height: 3rem;
      font-size: 2rem;
    ...
    /* ── Footer: compatto ── */
    .footer {
      padding: 1.75rem 1rem 5.5rem; /* spazio per FAB WhatsApp */
    }

    /* ── WhatsApp FAB: visibile solo su mobile ── */
    .whatsapp-fab {
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    /* ── Nasconde elementi superflui su mobile ── */
    .ornament {
      margin-bottom: 1.25rem;
    }

    /* ── Admin panel: input più grandi (evita zoom iOS) ── */
    .input, .select-field, .textarea-field {
      font-size: 1rem;
...
// ── Icone SVG WhatsApp e Telefono per nav mobile ──
        ...
        <div style={{
          ...
          {NAV_ITEMS.map(item => (
            <button
              key={item}
              style={{
                background: page === item ? "rgba(201,168,76,0.12)" : "none",
                border: "none",
                borderLeft: page === item ? "3px solid #c9a84c" : "3px solid transparent",
                color: page === item ? "#c9a84c" : "#ccc",
                cursor: "pointer",
                fontSize: "1rem",
                padding: "0.85em 1em",
              ...
              onClick={() => { navigateTo(item); setMenuOpen(false); }}
            >
              {item === "Home" && "🏠 "}
              {item === "Vendita" && "🚗 "}
              {item === "Noleggio" && "🔑 "}
  ...
  return (
    <div className="page-wrapper">
          ...
          <>
            ...
            {page === "Vendita" && (
              <section className="section">
                ...
                <AdvancedFilters filters={filters} onChange={setFilters} allTags={allTags} type="vendita" resultCount={filteredSale.length} />
            ...
            {page === "Noleggio" && (
              <section className="section">
                ...
                <AdvancedFilters filters={filters} onChange={setFilters} allTags={allTags} type="noleggio" resultCount={filteredRental.length} />
      ...
      <a
        ...
        href="https://wa.me/393930008654?text=Ciao%2C%20vi%20contatto%20dal%20sito%20per%20informazioni%20sulle%20auto"
        target="_blank"
